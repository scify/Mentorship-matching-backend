...
class MentorStorage {
    ...
    public function getDataForExportation() {
        return DB::select(DB::raw('select mentor.id, mentor.first_name, mentor.last_name, mentor.year_of_birth, 
          mentor.address, mentor.residence_name, mentor.email, mentor.linkedin_url, mentor.phone, mentor.cell_phone, 
          mentor.company_sector, mentor.job_position, mentor.job_experience_years, mentor.university_name, 
          mentor.university_department_name, mentor.skills, company.name as company_name, reference.name as reference_name,
          mentor.reference_text, 
          specialty.name as specialty_name, industry.name as industry_name, residence.name as residence_name_foreign, 
          university.name as university_name_foreign, education_level.name as education_level_name
