...
@else
    @if($loggedInUser->userHasAccessToCRUDMentorsAndMentees())
        ...
        <a target="_blank" href="{{route('showEditMentorForm', $mentorViewModel->mentor->id)}}" class="hidden secondItem menu-action"><i class="editIcon ion-edit"></i></a>
    ...
    @elseif($loggedInUser->userHasAccessToEditMentorsAndMentees())
        <a target="_blank" href="{{route('showEditMentorForm', $mentorViewModel->mentor->id)}}" class="hidden menu-action"><i class="editIcon ion-edit"></i></a>
    @endif
@endif
