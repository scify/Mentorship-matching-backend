            ...
            @if($loggedInUser->isAccountManager())
                ...
                <div class="panel">
                    ...
                    <div class="panel-body">
                        @if($pendingMentorshipSessionViewModelsForAccManager && $pendingMentorshipSessionViewModelsForAccManager->count() == 0)
                            You don't have any pending mentorship sessions assigned. You can view all the sessions you
                            participate in <a href="{{ route('showMentorshipSessionsForAccountManager') }}"> here. </a>
                        @else
                            You currently manage {{ $mentorshipSessionsNumForAccManager }} mentorship sessions. <a
                                    href="{{ route('showMentorshipSessionsForAccountManager') }}"> Click here </a> to
                            view them.
                        @endif
                    </div>
                </div>
        ...
        @if($loggedInUser->isMatcher())
            <div class="col-md-12">
                <div class="panel">
                    ...
                    <div class="panel-body">
                        There are <b>{{ $availableMentorsViewModelsNum }}</b> mentors available for matching in the
                        platform. <a href="{{ route('showAllMentors') }}"> Click here </a> to view all mentors.
                    </div>
                </div>
            </div>
        @endif
